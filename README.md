...
A robust, self-discovering Python 3 automation framework designed for Synology DSM environments. This solution automatically intercepts physical external USB hard drive insertions, dynamically discovers the target volume path, profiles the incoming
workload, handles fail-safe storage space auto-pruning, and executes a zero-dependency data replication strategy while streaming real-time telemetry back to Home Assistant.

## Core Pillars & Philosophy

* **Data Sovereignty:** Backups are written as plain-text, uncompressed, unencrypted directories on standard filesystems (`exFAT` or `ext4`). In a total catastrophic infrastructure failure, your data is accessible on virtually any device without
  proprietary recovery software.
...
Format your backup drives using **exFAT** (for universal cross-compatibility) or native Linux filesystems. Use your computer or Synology's External Devices tool to create an empty tracking file on the absolute root of the disk directory matching your
chosen rotation type:
...
To capture and display the structured telemetry sent by the script, create a Webhook-triggered automation in Home Assistant. This automation parses the separate `"status"` and `"message"` keys provided in the JSON payload, allowing you to route urgent errors directly to persistent alerts while sending standard progress milestones as low-priority push notifications to your mobile devices.

You can implement this by creating a new automation, switching the editor to **Edit in YAML**, and pasting the following configuration:

```yaml
alias: NAS Backup Notifications
description: Receives success/failure data from Linux script and emails it
triggers:
  - allowed_methods:
      - POST
    local_only: true
    trigger: webhook
    webhook_id: wfL7kWCER4R49ej4_nas_backup
conditions: []
actions:
  - action: notify.octabodegmail
    metadata: {}
    data:
      target: mark@octabode.com
      title: |
        {% if trigger.json.status == 'error' %}
          ❌ NAS Backup Error
        {% elif trigger.json.status == 'warning' %}
          ⚠️ NAS USB Backup Warning
        {% elif trigger.json.status == 'progress'%}
          💾 NAS USB Backup Progress
        {% elif trigger.json.status == 'success' %}
          ✅ NAS Backup Success
        {% endif %}
      message: |
        {% if trigger.json.status in ['warning', 'error'] %}
          <p style="color: #d32f2f; font-weight: bold;">An event requiring attention occurred during the USB automation:</p>
          <div style="background-color: #ffebee; border-left: 4px solid #d32f2f; padding: 10px; margin-bottom: 10px;">
            {{ trigger.json.message }}
          </div>
        {% elif trigger.json.status == 'success' %}
          <p>The USB drive may now be ejected using mobile DS File app or DSM Desktop.</p>
          <p>{{ trigger.json.message }}</p>
          <p><strong>Successfully backed up:</strong></p>
          <ul>
            <li>{{ trigger.json.message.split('Successfully backed up: ')[1] | replace(', ', '</li><li>') }}</li>
            </ul>
        {% else %}
          <p>NAS backup progress<p>
          <p>{{ trigger.json.message }}</p>
        {% endif %}
mode: queued
max: 10
